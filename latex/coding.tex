...
\begin{frame}
\frametitle{基本的BASH环境}
        \begin{enumerate}
        \item  export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:./
                添加运行时搜索路径
        \item ldconfig -d ./ 
                维护当前目录动态库版本.
        \item cp,scp, rsync
...
\begin{frame}
\frametitle{基础知识-pthread}

\begin{enumerate}
\item  线程操作, create, join, detach, exit
\item  锁,pthread_mutex_xxx, pthread_spin_xxx , pthread_rwlock_xxx
\end{enumerate}
...
\item 信号量 sem_xxx, semxxx
\item 共享内存 shm_xxx, shmxxx
\item 消息队列  mq_xxx msgxxx
\end{enumerate}
